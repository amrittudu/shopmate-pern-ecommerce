...
export const createProduct  = catchAsyncErrors( async (req, res, next) => {
    ...
    if( req.files && req.files.images ) {
        ...
        for (const image of images) {
            const result = await cloudinary.uploader.upload( image.tempFilePath, {
                folder : "shopmate_ecommerce/products",
            }) ;
    ...
    const product = await database.query(
        ...
        [name, description, price / 93.35, category, stock, JSON.stringify(uploadedImages), createdBy]
    );
...
export const fetchAllProducts = catchAsyncErrors( async (req, res, next) => {
    ...
    // query for fetching top rated products (products with average rating of 4 or higher)
    const topRatedQuery = `
    SELECT p.*,
    COUNT(r.id) AS review_count
    FROM products p
...
export const updateProduct = catchAsyncErrors( async (req, res, next) => {
    ...
    const result = await database.query(
        ...
        price = $3, category = $4, stock = $5 
        WHERE id = $6 RETURNING *`,
        [name, description, price / 93.35, category, stock, productId]
    );
